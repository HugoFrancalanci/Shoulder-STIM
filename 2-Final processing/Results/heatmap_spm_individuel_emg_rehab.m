% =========================================================================
% heatmap_spm_individuel_emg_rehab.m
% =========================================================================
...
%                (10 patients x 5 conditions, 4 muscles per cell). No FES vs
%                Rehab is excluded here — already covered (inverted) by
%                heatmap_spm_individuel_emg_noSEF.m (Rehab vs No FES).
...
%                    among the 5 compared conditions only (No FES and Rehab
%                    excluded from the ranking)
%                Figure 2 — % of patients (N=10) with a significant
%                    post-hoc vs Rehab, per condition and muscle.
...
%                Results are hardcoded from extract_emg_cycles_rehab.m
%                console output (LP_FREQ=6Hz, ANOVA RM + ttest_paired,
%                Bonferroni alpha=0.05/5, N=3 blocks per patient, exploratoire).
%                sig is all-zero here : the console run of
%                extract_emg_cycles_rehab.m found ANOVA significant for
%                many patient/muscle combinations, but zero significant
%                post-hoc vs Rehab across all patients/muscles/conditions
%                (all "n.s."). This is a genuine finding, not a missing
%                placeholder — see PATIENT_EXCEPTIONS/N=3 power limitation.
% -------------------------------------------------------------------------
...
% =========================================================================
% Résumé des résultats SPM1D individuels — EMG — reference Rehab
%
% Heatmap : patients (lignes) × conditions (colonnes, 5)
% Chaque cellule : 4 marqueurs (TRAPS, TRAPM, TRAPI, SERRA)
%   - Grand plein   = post-hoc significatif vs Rehab
%   - Petit gris    = n.s.
%
% Source : extract_emg_cycles_rehab.m — analyse SPM1D individuelle
%          ANOVA RM + post-hoc ttest_paired vs Rehab
%          Bonferroni α = 0.05/5 = 0.01 | N=3 blocs (exploratoire)
% =========================================================================
...
% -------------------------------------------------------------------------
% sig(patient, condition, muscle) = 1 si post-hoc significatif (vs Rehab)
% Conditions : 1=Min_fatigue 2=Min_stress 3=Random 4=Min_pulse_width 5=Min_force
% Muscles    : 1=TRAPS  2=TRAPM  3=TRAPI  4=SERRA
%
% Aucun post-hoc significatif trouve (sortie console verifiee) : ANOVA
% souvent significative (voir warnings/log), mais tous les t-tests
% apparies vs Rehab sont n.s. pour tous les patients/muscles/conditions.
% Matrice volontairement laissee a zero (resultat reel, pas un oubli).
% -------------------------------------------------------------------------
...
% du rang). Memes valeurs que heatmap_spm_individuel_kin_rehab.m (meme
% ordre de passage en session, independant de la mesure kin vs EMG).
...
% -------------------------------------------------------------------------
figure('Name','SPM1D individuel — significativite EMG vs Rehab', ...
...
title({'Analyse SPM1D individuelle — EMG — reference Rehab'; ...
...
figure('Name','SPM1D individuel — % patients significatifs EMG vs Rehab', ...
       'units','normalized','outerposition',[0.05 0.05 0.90 0.85],'Color','white');
...
title('% de patients (N=10) avec post-hoc significatif vs Rehab, par condition et muscle', ...
